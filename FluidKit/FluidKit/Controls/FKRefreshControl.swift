...
open class FKRefreshControl: UIRefreshControl {
  
  /// The shape layer to be animated
  open var indicatorLayer = CAShapeLayer()
  open fileprivate(set) var isAnimating = false
  
  open override var tintColor: UIColor! { didSet { self.indicatorLayer.strokeColor = self.tintColor?.cgColor } }
  ...
  /// Override to init your desired shape
  open func initShape() {
  ...
  open func scrollViewDidScroll(_ scrollView: UIScrollView) {
    if self.isRefreshing {
      ...
      self.updateProgress(progress: pullRatio)
    }
